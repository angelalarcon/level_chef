...
(function( window, $, undefined ){
     ...
     * ====================================================================== */
        ...
        /* filter isotope with the default filter selected */
        var filterDefaultValue = $(settings.filterContainer).find(settings.filter).filter('.selected').attr('data-filter');
        if( filterDefaultValue == undefined ){
            filterDefaultValue = '*';
        }
        ...
        function addWrapperForMargins(container){
            var wrapper = $('<div class="media-box-container"></div')
            				.css({
            						...
            						'margin-top'  : container.data('settings').verticalSpaceBetweenBoxes
            					});
            ...
            boxes.each(function(){
                ...
                if(boxImage.data('overlay-effect') != undefined){
                    ...
                    /* Set some CSS style for this effects */
                        ...
                        thumbOverlay.find('.mb-icon-plus, .mb-icon-link, .mb-icon-movie, .mb-icon-sound').css({ scale : 1.4 });
     ...
     * ====================================================================== */
        ...
        function updateFilterClasses(){
            ...
            boxes.filter( filter ).removeClass('hidden-media-boxes-by-filter');
            boxes.not( filter ).addClass('hidden-media-boxes-by-filter');
        }

        function filterIsotope( filterValue ){

            $container.isotopeMB({ filter: filterValue });

            updateFilterClasses();
            
            if( checkMinBoxesPerCategory()  ){
                /* it will load the missing boxes */
            }else{
                if( getLoadingBoxesInCurrentCategory().length > 0 ){
                    loading();
                }else{
                    fixLoadMoreButton();
                }
        ...
        $(settings.filterContainer).find(settings.filter).on('click', function(){
			var $this = $(this);
     ...
     * ====================================================================== */
        ...
        function checkMinBoxesPerCategory(){

            var boxesInCurrentCategory = getBoxesInCurrentCategory().length;

            /* Also check if there's boxes waiting to get load from that category, because maybe there isn't and there's no case to try to load them */
            if( boxesInCurrentCategory < settings.minBoxesPerCategory && hiddenBoxesWaitingToLoad().length > 0 ){
                /* Load the boxes that are missing */
                loadMore( settings.minBoxesPerCategory - boxesInCurrentCategory );
        ...
        function getBoxesInCurrentCategory(){
            var boxes = $container.find(itemSelector);   
        ...
        function getLoadingBoxesInCurrentCategory(){
            var boxes = getBoxesInCurrentCategory().not('.media-box-loaded');   
     ...
     * ====================================================================== */    
        ...
        $container.on( 'mouseenter.hoverdir, mouseleave.hoverdir', boxImageSelector, function(event){
            ...
            if( effect == 'push-up' || effect == 'push-up-100%' ){
                ...
                /* Effect of the icons */
                var icons = thumbnailOverlay.find('.mb-icon-plus, .mb-icon-link, .mb-icon-movie, .mb-icon-sound');
                if( eventType == 'mouseenter' ){
                    icons.css({ scale: 1.4 }); 
